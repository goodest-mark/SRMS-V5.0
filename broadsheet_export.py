...
from reportlab.platypus import PageBreak, Frame, PageTemplate, KeepInFrame
        ...
        if mark is not None and mark != '-' and mark != '':
            try:
                ...
                if point is not None:
                    total_points += point
                    count += 1
                # silently skip unknown grades
            except (ValueError, TypeError):
                # silently skip non‑numeric marks
                continue
    if count == 0:
        return "N/A"
...
# ------------------------------------------------------------
# Ensure school profile is always available
# ------------------------------------------------------------
def _ensure_school_profile(meta):
    """If school_profile is missing, fetch it from SystemState."""
    if 'school_profile' not in meta or not meta['school_profile']:
        try:
            from system_state import SystemState
            meta['school_profile'] = SystemState.get_school_profile() or {}
        except ImportError:
            meta['school_profile'] = {}
    return meta
...
def to_excel(parent, data):
    ...
    data['meta'] = _ensure_school_profile(data['meta'])
...
def to_pdf(parent, data):
    ...
    try:
        ...
        def _header_footer(canvas, doc):
            canvas.saveState()
            # Header band height. Content is wrapped in a KeepInFrame below so
            # that if it ever runs slightly taller than this (e.g. a longer
            # contact-info line wraps to an extra row) it gets shrunk to fit
            ...
            # Wrap in KeepInFrame(mode='shrink') so that if the content is
            # ever taller than header_height (long school name, long address,
            # website added, etc.) it gets scaled down to fit rather than
            # being silently dropped by Frame.addFromList().
            safe_header = KeepInFrame(doc.width, header_height, [header], mode='shrink')
            remaining = header_frame.addFromList([safe_header], canvas)
            if remaining:
                print(f"[WARNING] Header band did not fit and was skipped "
                      f"({len(remaining)} flowable(s) left over). "
                      f"Consider increasing header_height.")

            # Divider lines
            band_top = doc.height + doc.topMargin
            band_bottom = band_top - header_height + 0.1 * inch
        ...
        # Build the document – the template is now active via setPageTemplate
        doc.build(elements)
